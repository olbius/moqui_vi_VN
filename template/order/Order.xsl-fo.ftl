...
<fo:root xmlns:fo="http://www.w3.org/1999/XSL/Format" font-family="${fontArial}, sans-serif" font-size="10pt">
    ...
    <fo:page-sequence master-reference="letter-portrait" id="mainSequence">
        <fo:static-content flow-name="xsl-region-before">
            <fo:block font-size="14pt" text-align="center" margin-bottom="0.1in"><#if firstPartInfo.isCustomerInternalOrg>${ec.l10n.localize("Purchase Order")}<#else>${ec.l10n.localize("Sales Order")}</#if> #${orderId}</fo:block>
            <fo:table table-layout="fixed" margin-bottom="0.1in" width="7.5in">
                <fo:table-body><fo:table-row>
                    <fo:table-cell padding="3pt" width="3.75in">
                        ...
                        <fo:block><@encodeText (firstPartInfo.vendorDetail.partyName)!""/></fo:block>
                        <fo:block font-weight="bold">${ec.l10n.localize("Customer")}</fo:block>
                        <fo:block><@encodeText (firstPartInfo.customerDetail.partyName)!""/></fo:block>
                    </fo:table-cell>
        ...
        <fo:static-content flow-name="xsl-region-after" font-size="8pt">
            <fo:block border-top="thin solid black">
                ...
                <fo:block text-align="center">${ec.l10n.localize("Order")} #${orderId} -- <#if orderHeader.placedDate??>${ec.l10n.format(orderHeader.placedDate, "dd MMM yyyy")}<#else>${ec.l10n.localize("Not yet placed")}</#if> -- ${ec.l10n.localize("Page")} <fo:page-number/></fo:block>
            </fo:block>
        </fo:static-content>
            ...
            <#list orderPartInfoList as orderPartInfo>
                    ...
                    <fo:table-body><fo:table-row>
                        <fo:table-cell padding="3pt" width="2in">
                            ...
                            <#if orderPart.otherPartyOrderId?has_content>
                                <fo:block font-weight="bold"><#if orderPartInfo.isVendorInternalOrg>${ec.l10n.localize("Customer Order")}<#else>${ec.l10n.localize("Vendor Order")}</#if> #</fo:block>
                        ...
                        <fo:table-cell padding="3pt" font-size="10pt" width="3.5in">
                            ...
                            <#if contactInfo.postalAddress?has_content>
                                <#if contactInfo.postalAddress.toName?has_content || contactInfo.postalAddress.attnName?has_content>
                                    <#if contactInfo.postalAddress.toName?has_content><fo:block font-weight="bold">${ec.l10n.localize("To")}: <@encodeText contactInfo.postalAddress.toName/></fo:block></#if>
                                    <#if contactInfo.postalAddress.attnName?has_content><fo:block font-weight="bold">${ec.l10n.localize("Attn")}: <@encodeText contactInfo.postalAddress.attnName/></fo:block></#if>
                                <#else>
                                    <fo:block font-weight="bold"><@encodeText orderPartInfo.customerDetail.partyName!""/></fo:block>
                                </#if>
                ...
                <fo:table table-layout="fixed" width="100%">
                    <fo:table-header font-size="9pt" border-bottom="solid black">
                        ...
                        <#if showUpc><fo:table-cell width="1in" padding="${cellPadding}"><fo:block>${ec.l10n.localize("UPC")}</fo:block></fo:table-cell></#if>
                ...
                <#if orderPartInfo.orderPart.giftMessage?has_content>
                    <fo:block margin-top="0.2in" font-weight="bold">${ec.l10n.localize("Gift Message")}</fo:block>
